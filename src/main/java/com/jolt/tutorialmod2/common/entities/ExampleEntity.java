package com.jolt.tutorialmod2.common.entities;
...
public class ExampleEntity extends AnimalEntity implements IAnimatedEntity{
		...
		{
			if (event.isWalking()) {
				controller.setAnimation(new AnimationBuilder().addAnimation("animation.tutorial.walk").addAnimation("animation.tutorial.walk",true));	
				return true;
			}else {
				controller.setAnimation(new AnimationBuilder().addAnimation("animation.tutorial.idle").addAnimation("animation.tutorial.walk",true));
				return true;
				}
			}
		...
		private <E extends Entity> void particleListener(ParticleKeyFrameEvent<E> event) {
			if (event.effect.equals("test")) {
				event.getEntity().getEntityWorld().addParticle(new BlockParticleData(null, getBlockState()),
